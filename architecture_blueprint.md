...
6. **Strict `apisession` Token Auto-Capture, Multi-Route History Scrubbing & Immediate 1-Tap App Launch (Flutter Mobile & Web Portal)**:
   ...
   - On both the web callback routes (`/api/auth/icici-callback`, `/callback`) and the Web Portal root route (`/`), callbacks process `apisession` case-insensitively, supporting GET query parameters and POST request bodies with Base64 padding auto-recovery. The client-side logic renders the session key in a visible, styled on-screen `<input>` element with an instant **`📋 Copy Session Token`** button (utilizing synchronous DOM input selection + `execCommand('copy')` with `readonly` bypass, parallel modern `navigator.clipboard.writeText`, and `Permissions-Policy: clipboard-write=(self)`) followed by an **immediately active, vibrant `📱 1-Tap Open in StokVigil App →` button** that triggers the `stokvigil://breeze-callback` deep link. `window.history.replaceState` scrubs sensitive query parameters from the browser address bar and history to prevent credential leakage in logs, bookmarks, or referrers, enforced alongside strict Content Security Policies (CSP) and `X-Frame-Options: DENY`.
